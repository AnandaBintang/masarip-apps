...
@section('content')
    <form method="POST" action="{{ route('user.update', $user->id) }}" id="user-form">
        @csrf
        @method('PUT')
        <h3>Edit Pengguna</h3>
        ...
        <div style="margin-bottom: 10px;">
            ...
            <input name="name" type="text" id="user-nama" style="width: 100%;" value="{{ old('name', $user->nama) }}" />
            @error('nama')
        ...
        <div style="margin-bottom: 10px;">
            ...
            <select name="role" id="user-role" style="width: 100%;">
                ...
                <option value="staff" {{ old('role', $user->role) == 'staff' ? 'selected' : '' }}>Staff Umum</option>
            </select>
            @error('role')
                <div style="color: red;">{{ $message }}</div>
            @enderror
        ...
        <div style="margin-bottom: 15px;">
            <label for="user-password">Password (Kosongkan jika tidak ingin mengubah):</label>
